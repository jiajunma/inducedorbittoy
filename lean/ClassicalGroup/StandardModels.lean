...
@[simp] theorem dot_zero_left {ι : Type*} [Fintype ι] (x : ι → ℂ) :
    dot (fun _ => 0) x = 0 := by
  simp [dot]

@[simp] theorem dot_zero_right {ι : Type*} [Fintype ι] (x : ι → ℂ) :
    dot x (fun _ => 0) = 0 := by
  simp [dot]
...
@[simp] theorem dotForm_apply {ι : Type*} [Fintype ι] (x y : ι → ℂ) :
    dotForm x y = dot x y :=
  rfl
...
theorem dotForm_nondegenerate {ι : Type*} [Fintype ι] [DecidableEq ι] :
  ...
  · intro x hx
    ...
    simpa [dot_single_right] using h
  · intro y hy
    ext i
    have h := hy (Pi.single i 1)
    simpa [dot_single_left] using h
...
@[simp] theorem orthForm_apply (p q : ℕ) (x y : OrthVec p q) :
  ...
  nondegenerate := by
    ...
    · intro x hx
      ...
      · have h := hx (Pi.single i 1, 0)
        simpa [orthForm_apply, dot_single_right, dot_zero_right] using h
      · have h := hx (0, Pi.single i 1)
        have hneg : -x.2 i = 0 := by
          simpa [orthForm_apply, dot_single_right, dot_zero_right, dot_zero_left] using h
        exact neg_eq_zero.mp hneg
    · intro y hy
      ext i
      · have h := hy (Pi.single i 1, 0)
        simpa [orthForm_apply, dot_single_left, dot_zero_left] using h
      · have h := hy (0, Pi.single i 1)
        have hneg : -y.2 i = 0 := by
          simpa [orthForm_apply, dot_single_left, dot_zero_right, dot_zero_left] using h
        exact neg_eq_zero.mp hneg
  eps_symm := by
    intro x y
    simp [orthForm_apply, dot_comm y.1 x.1, dot_comm y.2 x.2]
...
def orthJ (p q : ℕ) : JStructure Sign.pos (OrthVec p q) Sign.pos where
  ...
  sq := by intro v; ext i <;> simp [pairConjSemilinearEquiv]
  preserves_form := by
    intro u v
...
def orthL (p q : ℕ) : LStructure Sign.pos (OrthVec p q) Sign.pos where
  ...
  sq := by intro v; ext i <;> simp [prodSignLinearEquiv]
  preserves_form := by
    intro u v
...
@[simp] theorem sympForm_apply (r : ℕ) (x y : PairVec r) :
...
instance sympFormedSpace (r : ℕ) : FormedSpace Sign.neg (PairVec r) where
  ...
  nondegenerate := by
    ...
    · intro x hx
      ext i
      · have h := hx (0, Pi.single i 1)
        simpa [sympForm_apply, dot_single_right] using h
      · have h := hx (Pi.single i 1, 0)
        have hneg : -x.2 i = 0 := by
          simpa [sympForm_apply, dot_single_right, dot_single_left] using h
        exact neg_eq_zero.mp hneg
    · intro y hy
      ext i
      · have h := hy (0, Pi.single i 1)
        ...
        exact neg_eq_zero.mp hneg
      · have h := hy (Pi.single i 1, 0)
        simpa [sympForm_apply, dot_single_left] using h
  eps_symm := by
    intro x y
    simp [sympForm_apply, dot_comm y.1 x.2, dot_comm y.2 x.1]
...
def sympL (r : ℕ) : LStructure Sign.neg (PairVec r) Sign.neg where
  ...
  sq := by intro v; ext i <;> simp [rotateLinearEquiv]
  preserves_form := by
    intro u v
...
@[simp] theorem cstarForm_apply (a b : ℕ) (x y : CstarVec a b) :
...
instance cstarFormedSpace (a b : ℕ) : FormedSpace Sign.neg (CstarVec a b) where
  ...
  eps_symm := by
    intro x y
    simp [cstarForm_apply, sympForm_apply, dot_comm y.1.1 x.1.2,
...
def cstarJ (a b : ℕ) : JStructure Sign.neg (CstarVec a b) Sign.neg where
  ...
  sq := by
    intro v
    ext i <;> simp [cstarJSemilinearEquiv, quatJEquiv]
  preserves_form := by
    intro u v
...
def cstarL (a b : ℕ) : LStructure Sign.neg (CstarVec a b) Sign.pos where
  ...
  sq := by intro v; ext <;> simp [prodSignLinearEquiv]
  preserves_form := by
    intro u v
...
@[simp] theorem dstarForm_apply (r : ℕ) (x y : PairVec r) :
    ...
    · intro x hx
      ...
      · have h := hx (0, Pi.single i 1)
        have hmul : (-Complex.I) * x.1 i = 0 := by
          simpa [dstarForm_apply, dot_single_right] using h
        rcases mul_eq_zero.mp hmul with hI | hx0
        · exact False.elim (neg_I_ne_zero hI)
        · exact hx0
      · have h := hx (Pi.single i 1, 0)
        have hmul : (-Complex.I) * x.2 i = 0 := by
          simpa [dstarForm_apply, dot_single_right, dot_single_left] using h
        rcases mul_eq_zero.mp hmul with hI | hx0
        · exact False.elim (neg_I_ne_zero hI)
        · exact hx0
    · intro y hy
      ...
      · have h := hy (Pi.single i 1, 0)
        have hmul : (-Complex.I) * y.2 i = 0 := by
          simpa [dstarForm_apply, dot_single_left] using h
        rcases mul_eq_zero.mp hmul with hI | hy0
        · exact False.elim (neg_I_ne_zero hI)
        · exact hy0
  eps_symm := by
    intro x y
    simp [dstarForm_apply, dot_comm y.1 x.2, dot_comm y.2 x.1]
    ring
...
def dstarJ (r : ℕ) : JStructure Sign.pos (PairVec r) Sign.neg where
  ...
  sq := by intro v; ext i <;> simp [quatJEquiv]
  preserves_form := by
    intro u v
