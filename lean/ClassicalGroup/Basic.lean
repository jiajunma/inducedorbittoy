...
`/Users/hoxide/mydoc/inducedorbittoy/classicalgroup.md`.
...
@[simp] theorem toComplex_pos : Sign.pos.toComplex = (1 : ℂ) := rfl

@[simp] theorem toComplex_neg : Sign.neg.toComplex = (-1 : ℂ) := rfl

@[simp] theorem toComplex_mul (a b : Sign) :
...
@[simp] theorem coe_apply (J : JStructure eps V jSign) (v : V) :
    J v = J.toSemilinearEquiv v := rfl

@[simp] theorem toSemilinearMap_apply (J : JStructure eps V jSign) (v : V) :
...
@[simp] theorem coe_apply (L : LStructure eps V lSign) (v : V) :
    L v = L.toLinearEquiv v := rfl

@[simp] theorem toLinearMap_apply (L : LStructure eps V lSign) (v : V) :
