import ClassicalGroup.StandardModels
...
private noncomputable def BDPlusFixedInnerCore
  ...
  conj_inner_symm := by
    ...
    simp at hsym ⊢
    exact congrArg Complex.re hsym.symm
  re_inner_nonneg := by
    intro x
    by_cases hx : x = 0
    · subst x
...
private noncomputable def BDMinusFixedInnerCore
  ...
  conj_inner_symm := by
    intro x y
    have hsym := FormedSpace.eps_symm (eps := Sign.pos) (V := V) x.1.1 y.1.1
    have h := congrArg Complex.re hsym.symm
    simp at h ⊢
    exact h
  re_inner_nonneg := by
    intro x
  ...
  have hB : Bpull = FormedSpace.B Sign.neg V := by
    ...
    rcases i with (_ | _) | (_ | _) <;> rcases j with (_ | _) | (_ | _) <;>
      simp [Bpull, toLinearEquiv, FormedSpace.B, cstarFormedSpace, cstarForm_apply,
        ...
        cstarCoord_neg_e, cstarCoord_neg_f, dot_pi_single_single]
    all_goals
      split_ifs <;> ring
  intro u v
  have h := congrArg (fun B : LinearMap.BilinForm ℂ V => B u v) hB
...
private theorem cstarAdaptedBasis_of_quaternionic_families
  ...
  have hMinus_J_e :
    ...
    by_cases hij : i = j
    · subst j
      simpa [hMinus_pair i i] using hsym
    · have hji : j ≠ i := fun h => hij h.symm
      simpa [hij, hji, hMinus_pair j i] using hsym
  have hpos_neg :
...
private theorem dstarAdaptedBasis_of_i_orthonormal
  ...
  have hJ_e :
    ...
    by_cases hij : i = j
    · subst j
      simpa [hpair i i] using hsym
    · have hji : j ≠ i := fun h => hij h.symm
      simpa [hij, hji, hpair j i] using hsym
