...
In \cite{ArPro, ArUni}, Arthur predicted the existence of a collection of automorphic representations, the so-called unipotent automorphic representations, with the purpose of building arbitrary automorphic representations (from tempered and unipotent ones). In particular, it suggested the existence of certain collections of representations of reductive algebraic groups over $\BR$ or $\BC$. Arthur’s desired representations, the special unipotent representations in the title of this paper, were defined by Barbasch-Vogan in \cite{BVUni} for groups over $\BC$, and by Adams-Barbasch-Vogan (abbreviated as ABV hereafter) in \cite[Chapter 27]{ABV} for groups over $\BR$.
...
Combining with Barbasch's work in \cite{B.Class} for complex classical groups and the authors' work in \cite{BMSZ2} for simple linear Lie groups of type $A$, we may further conclude the unitarity of special unipotent representations for any real form of a connected reductive complex Lie group of a classical type. See \Cref{thmA'} for the precise statement. 
We remark that Adams, Miller, van Leeuwen and Vogan have recently established the same unitarity result in the case of real exceptional groups, with the Atlas algorithm of Adams-van Leeuwen-Trapa-Vogan being a key tool. See \cite{AMVV}.
...
classical groups (see for example, \cite{HoweRank,Li89}). Since then there have been many attempts to construct special unipotent representations (or more generally the so-called unipotent representations, see \cite{VoBook})
...
In the archimedean case, we refer the reader to several works \cite{Mo17,MR18a,MR18b,MR19,MR20} of M{\oe}glin, and M{\oe}glin and Renard  for results in this direction. 
...
Let $\star$ be one of the 10 labels, let $G$ be a classical Lie group of type $\star$, and let $\check G$ be its Langlands dual group (\cite{Borel}) or a variation, as in the following table ($n,p,q\in \bN:=\{0,1,2, \dots\}$).
\be\label{tableg}
  \begin{aligned}
...
Following Barbasch-Vogan (\cite{BVUni}) and  Adams-Barbasch-Vogan (\cite[Chapter 27]{ABV}), define the set of the special unipotent representations of $G$
 attached to $\ckcO$ by
%\[
...
\end{equation*}
 ...
 \ee
 for some Young diagram $\check \CO'_\mathrm b$. This is automatic when  $\star\in \set{B, C,\wtC, C^{*},D,D^{*}}$, and is necessary when $\star \in\{A, \wtA\}$ and the set $\Unip_{\ckcO}(G)$ is nonempty (see \cite[Theorem 2.16]{BMSZa}).
...
Our second key observation is that the descent map carries with it a host of geometric information. 
...
\begin{remark} By the fundamental result of Schmid and Vilonen in \cite{SV}, the (weak) associated cycle agrees with the wave
...
%In particular, $\mathrm{AC}_{\CO}(\pi)$ is irreducible, for any $\pi \in \mathrm{Unip}_{\check \CO}(G)$.
\end{thm}
...
We now give an outline on the organization of this article. In Section \ref{sec:bip}, we first review the combinatorics (including the counting) of special unipotent representations from our earlier paper \cite{BMSZa}. This includes the notion of painted bipartitions as well as the descent of a painted bipartition. The key combinatorial object is an  extended set $\mathrm{\PBPes}(\check \CO)$ of painted bipartitions attached to $\check \CO$. This will be our parameter set for special unipotent representations attached to $\check \CO$. Given 
...
\]
Similar inductive procedures will be adopted when we define the associated cycle and the combinatorial cycle attached to $\uptau$ in 
...
Denote by $G_{\C}$ the isometry group of $ (V, \la\,,\,\ra)$, which is a complex orthogonal group if $\epsilon=1$ and a complex symplectic group if $\epsilon=-1$. Respectively denote by  $G_{\C}^{J}$  and  $G_{\C}^{L}$ the centralizer of $J$ and $L$ in  $G_{\C}$. Then $G_{\C}^{J}$ is a real classical group isomorphic to
\be\label{realg}
 \left\{
     \begin{array}{ll}
...
\subsection{The moment maps and descent of a $K_{\C}$-orbit in $\p$}\label{secmmap}
...
Recall the well-known identification of a nilpotent orbit in $\g_\sfss$ with its Young diagram (see \cite{CM}). 
Via the notion of the naive descent of a Young diagram in Section \ref{sec:desc}, we may define the naive descent $\DD_\mathrm{naive}(\CO)$ of $\CO\in \Nil(\g_\sfss)$ at the level of Young diagrams. We record the following lemma for later use.
...
%{\color{red} \delta:=  \abs{\sfss'}-(\abs{\sfss}-\mathbf c_{1}(\CO)))\geq 0.}
\]
When this is the case, the Young diagram of $\DD_{\mathsf s'}^{\mathsf s}(\CO)\in \Nil(\g_{\sfss'})
$ is obtained from that of $\DD_\mathrm{naive}(\CO)$ by adding $\delta$ boxes in the first column.
\end{lem}
\begin{proof}
...
\]
Then $\star'=\star''$, and there is a  split classical signature $\sfss_0=(\star_0, \kappa, \kappa)$ such that
\begin{itemize}
\item $\star_0=\dot \star$;
\item $(p', q')+(\kappa, \kappa )=(p'', q'')$.
\end{itemize}
...
For every $g\in G_{\sfss'}$, write $g^\triangle:= g^- g$ (see \eqref{isosfss1}), which is an element of $R_{\dsfss}$.
...
Recall that $\sfss'=(\star', p',q')$. Let $\CO'\in\Nil(\g_{\sfss'})$, as before. Let $\sfss''$ be a classical signature of the form $(\star',p'+\kappa,q'+\kappa)$, where 
\begin{itemize}
    \item $\kappa\in \bN^+$ when $\star'\in \{B,C,\wtC, D\}$, and 
    \item $\kappa\in 2 \bN^+$ when $\star'\in \{C^*,D^*\}$. 
\end{itemize}
Then $\g_{\sfss''}$ has a Levi subalgebra of the form $\gl_{\kappa}(\C)\times \g_{\sfss'}$. Identify $\CO'$ with the nilpotent orbit $\{0\}\times \CO'$ in $\gl_{\kappa}(\C)\times \g_{\sfss'}$,
and put
\be\label{induo0}
\CO'':=\Ind_{\sfss'}^{\sfss''}(\CO'):=\Ind_{\gl_{\kappa}(\C)\times \g_{\sfss'}}^{\g_{\sfss''}}(\CO')\in \Nil(\g_{\sfss''})
\ee
for the induced nilpotent orbit, namely the unique nilpotent orbit in $\Nil(\g_{\sfss''})$ that contains a non-empty Zariski open subset of  $
  \CO'+\mathfrak n_0$. Here  $\mathfrak n_0$ is the nilpotent radical of a parabolic subalgebra of $\g_{\sfss''}$ that contains $\gl_{\kappa}(\C)\times \g_{\sfss'}$ as a Levi factor. See \cite{LS} or \cite[Section 7.1]{CM}. 


We assume that 
\[
\kappa \geq \mathbf c_2(\CO'),
\]
and put 
\be \label{eq:tzero}
  \delta_0:=\kappa -\mathbf c_1(\CO'). 
\ee
...
\begin{itemize}
    ...
    \item if $\star'\in \set{B,D, C^*}$,
    then
    \[
      (\bfcc_1(\cOpp), \bfcc_2(\cOpp),\bfcc_3(\cOpp))
      =\begin{cases}
        (\kappa,\kappa,\bfcc_1(\cOp)),     & \text{if $\delta_0\geq 0$ is even}; \\
          (\kappa+1,\kappa-1,\bfcc_1(\cOp)), & \text{if $\delta_0\geq 0$ is odd};  \\
          (\bfcc_1(\cOp),\kappa,\kappa),     & \text{if $\delta_0<0$};             \\
      \end{cases}
    \]
    \item  if $\star'\in \set{C,\wtC, D^*}$,
    then
    \[
      (\bfcc_1(\cOpp), \bfcc_2(\cOpp),\bfcc_3(\cOpp))
      =\begin{cases}
        (\kappa,\kappa,\bfcc_1(\cOp)),     & \text{if $\delta_0\geq 0$}; \\
        (\bfcc_1(\cOp),\kappa,\kappa), & \text{if $\delta_0 < 0$ is even};  \\
        (\bfcc_1(\cOp),\kappa +1,\kappa-1),     & \text{if $\delta_0<0$ is odd}.             \\
      \end{cases}
    \]
\end{itemize}
...
\]
Let $\sOp\in
\SYD_{\sfss'}(\cOp)$. We only need to define $\Ind_{\sfss'}^{\sfss''}(\sO')\in \Z[\SYD_{\sfss''}(\cO'')]$. Note that $\delta_0$ is even when $\star\in \{C^*, D^*\}$, as the set $\SYD_{\sfss'}(\cOp)$ is non-empty. 

\medskip 

{\bf The case when  $\star' \in \set{B,D, C^{*}}$.}
In this case, we define 
\begin{equation}\label{eq:IndSYD1}
      \Ind_{\sfss'}^{\sfss''}(\sOp): =
      \begin{cases}
        \sOp\circledast (\frac{\delta_0}{2},\frac{\delta_0}{2})\circledast (0,0),
         & \text{if $\delta_0 \geq 0$ is even}; \\
        2  \left(\sOp\circledast (\frac{\delta_0-1}{2},\frac{\delta_0-1}{2})\circledast (1,1) \right),
         & \text{if $\delta_0\geq 0$ is odd};   \\
        \sum_{a,b\in \bN, a+b=-\delta_0}
        (\Lambda_{(a,b)}\sOp)\circledast(0,0)\circledast (a,b),
         & \text{if $\star'\in \{B,D\}$ and $\delta_0<0$};
         \\
         \sum_{a,b\in 2\bN, a+b=-\delta_0}
        (\Lambda_{(a,b)}\sOp)\circledast(0,0)\circledast (a,b),
         & \text{if $\star'=C^*$ and $\delta_0<0$}.
      \end{cases}
\end{equation}
...
{\bf The case when  $\star' \in \set{C,\wtC, D^{*}}$.}
In this case, we define 
\begin{equation}\label{eq:IndSYD2}
      \Ind_{\sfss'}^{\sfss''}(\sOp) :=
      \begin{cases}
        \sum_{a, b\in \bN, a+b=\delta_0}\sOp\circledast (a,b)\circledast (0,0),
           & \text{if $\star'\in \{C,\wtC\}$ and $\delta_0\geq 0$}; \\
            \sum_{a, b\in 2 \bN, a+b=\delta_0}\sOp\circledast (a,b)\circledast (0,0),
           & \text{if $\star'=D^*$ and $\delta_0\geq 0$}; \\
        (\Lambda_{(\frac{-\delta_0}{2},\frac{-\delta_0}{2})}\sOp) \circledast (0,0)
        \circledast (\frac{-\delta_0}{2},\frac{-\delta_0}{2}),
         & \text{if $\delta_0 < 0$ is even};  \\
        2\left((\Lambda_{(\frac{1-\delta_0}{2},\frac{1-\delta_0}{2})}\sOp)
        \circledast (1,1) \circledast (\frac{1-\delta_0}{2},\frac{1-\delta_0}{2})\right),
            & \text{if $\delta_0<0$ is odd}.             
      \end{cases}
\end{equation}
...
\[
  \sfss_0 :=
  \begin{cases}
     (D, \kappa, \kappa ),& \textrm{if $\star'=B$};\\
     (\star',\kappa, \kappa),\quad & \textrm{otherwise}.
  \end{cases}
\]
As in   
\eqref{parabolic}, 
...
\[
  R_{\sfss_0} \cong
  \begin{cases}
     \GL_{\kappa}(\R),\quad & \textrm{if $\star\in \{B,D,C\}$};\\
     \widetilde \GL_{\kappa}(\R),\quad & \textrm{if $\star=\wtC$};\\
     \GL_{\frac{\kappa}{2}}(\bH),\quad & \textrm{if $\star\in \{C^*,D^*\}$}.
  \end{cases}
\]


\begin{prop}\label{lem:ind00}
Let $\pi'$ be an $\CO'$-bounded Casselman-Wallach representation of $G_{\sfss'}$ that extends to a representation of the Levi subgroup $G_{\sfss'}\cdot R_{\sfss_0}$ (still dented by $\pi'$). View it  as a representation of $P_{\sfss'',\sfss_0}$ through the trivial action of the unipotent radical. Then the (normalized smooth) parabolically induced representation $\Ind_{P_{\sfss'',\sfss_0}}^{G_{\sfss''}} (\pi')$ is $\CO''$-bounded, and 
\be\label{wac0}
...
\begin{proof}
%By the result of Schmid-Vilonen \cite{SV},  the weak associated cycle agrees with the wavefront cycle under the Kostant-Sekiguchi correspondence. {\color{red} On the other hand, the wavefront cycle for a parabolically induced representation is calculated by Barbasch in \cite[Corollary~5.0.10]{B.Orbit}, via induction of nilpotent orbits. From this result, the first assertion follows. For (wavefront cycle counterpart of) the left hand of \eqref{wac0}, we combine the result of Barbasch with the explicit description of parabolic induction of nilpotent orbits (see \Cref{prop:A6}, and \Cref{prop:A7}). The equality in \eqref{wac0} is then verified case by case. We omit the details.}
By the result of Schmid-Vilonen (\cite{SV}),  the weak associated cycle agrees with the wave
front cycle under the Kostant-Sekiguchi
correspondence.  On the other hand, the wavefront cycle for a parabolically induced representation is described by Barbasch in \cite[Corollary~5.0.10]{B.Orbit} via induction of nilpotent orbits. In Propositions \ref{prop:A6} and \ref{prop:A7} of \Cref{sec:orbitinduce}, we  explicitly determine the induced nilpotent orbits and their multiplicities. One may then read off the signed Young diagrams of the nilpotent $K_{\sfss'',\C}$-orbits occurring in the left-hand side of \eqref{wac0}.  Together with the computed multiplicities, the proposition follows. %\Cref{lem:ind00}.Then it is routine to   From this result, the first assertion follows. For (wavefront cycle counterpart of) the left hand of \eqref{wac0}, we combine the result of Barbasch with the explicit description of parabolic induction of nilpotent orbits (see \Cref{prop:A6}, and \Cref{prop:A7}). The equality in \eqref{wac0} is then verified case by case. We omit the details.}
\end{proof}
...
\begin{defn}
An nilpotent orbit $\CO\in \Nil(\g_\mathsf s)$ is good for $\DD_{\mathsf s'}^{\mathsf s}$ if it is regular for $\DD_{\mathsf s'}^{\mathsf s}$ (Definition \ref{defn:regular}) and satisfies the following additional condition:
\[
\begin{cases}
...
Theorem \ref{thm:GDS.AC} is obviously true when $\abs{\sfss}\leq 1$. Thus  we  assume in the rest of this subsection that $\abs{\sfss}\geq 2$.
In this remaining case, $\kappa>0$. Indeed, put
\[
  \delta:=\abs{\sfss'}-\abs{\DD_\mathrm{naive}(\CO)}.
\]
Since $\abs{\DD_\mathrm{naive}(\CO)}=\abs{\sfss}-\mathbf c_1(\CO)$, the goodness conditions imply $\kappa=\mathbf c_1(\CO)-1$ if $\star\in\{B,D\}$, $\kappa=\mathbf c_1(\CO)+1-\delta$ with $\delta\in\{0,1\}$ if $\star\in\{C,\widetilde C\}$, and $\kappa=\mathbf c_1(\CO)$ if $\star\in\{C^*,D^*\}$. In the first case, $\mathbf c_1(\CO)>\mathbf c_2(\CO)$ and $\abs{\sfss}\geq 2$ force $\mathbf c_1(\CO)\geq 2$.
As in \eqref{dotp}, we have that
\[
 \dot p=\dot q= \begin{cases}
...
(a) If $ \star\in \{B,D\}$,  then
\[
   \sum_{\sfss_1\in S_{\star, \abs{\sfss}}}  \left(\check \vartheta^{\sfss'', \CO''}_{\sfss_1, \CO}(\check \vartheta^{\sfss_1, \CO}_{\sfss',\CO'}(\mathrm{AC}_{\CO'}(\pi')))\right)^{\mathrm{weak}}=2\cdot \left( \mathrm{AC}_{\CO''}(\Ind_{P_{\sfss'',\sfss_0}}^{G_{\sfss''}} (\pi'))\right)^{\mathrm{weak}}.
        \]
...
(d) If $\star=C^*$, then
\[
  \sum _{\sfss_1\in S_{\star, \abs{\sfss}} } \left(\check \vartheta^{\sfss'', \CO''}_{\sfss_1, \CO}(\check \vartheta^{\sfss_1, \CO}_{\sfss',\CO'}(\mathrm{AC}_{\CO'}(\pi')))\right)^{\mathrm{weak}}=\left( \mathrm{AC}_{\CO''}(\Ind_{P_{\sfss'',\sfss_0}}^{G_{\sfss''}} (\pi'))\right)^{\mathrm{weak}}.
\]
\end{prop}
\begin{proof}
...
Then
 \begin{eqnarray*}
  && \left( \sum_{\sfss_1\in S_{\star, \abs{\sfss}} } \mathrm{AC}_{\cO''}( \Thetab^{\mathsf s''}_{\mathsf s_1}(\Thetab_{\mathsf s'}^{\mathsf s_1}(\pi')))\right)^{\mathrm{weak}}
\\
&=& \left( \sum_{\sfss_1\in S_{\star, \abs{\sfss}}}   \ckvartheta^{\sfss'', \CO''}_{\sfss_1, \CO}(\ckvartheta^{\sfss_1, \CO}_{\sfss',\CO'}(\mathrm{AC}_{\CO'}(\pi')))\right)^{\mathrm{weak}}.
\end{eqnarray*}
...
\label{sec:orbitinduce}
\label{app:induced-orbit}

\subsection{Parabolic induction of nilpotent orbits and multiplicities}
\label{app:induction-def}

Let $G$ be a real reductive group with Lie algebra ${\mathfrak g_\R}$, and let $P\subset G$ be a parabolic subgroup with Levi decomposition $P=M\ltimes U$. Write $\mathfrak m_\R:=\Lie(M)$, $\mathfrak u_\R:=\Lie(U)$, and 
%, and view $\mathfrak l_\R$ as a subalgebra of $\mathfrak g_\R$ via the Levi decomposition. 
let $\sO_0\subset\mathfrak m_\R$ be a nilpotent $M$-orbit. %, regarded as a subset of $\mathfrak g_\R$ through this inclusion.

\begin{defn}\label{def:Ind-orbit}
A nilpotent $G$-orbit $\sO\subset \g_\R$ is said to be induced  from $\sO_0$ if 
\[
\sO \cap (\sO_0+\mathfrak u_\R)
\]
is non-empty and open in $\sO_0+ \mathfrak u_\R$ 
\end{defn}

Let  $\sO$ be a nilpotent $G$-orbit induced  from $\sO_0$ (such an orbit always exists). 
%{\color{red} It is clear that a $G$-orbit $\sO\subset\Ind_P^G(\sO_0)$ is  an induced orbit from $\sO_0$ via $P$ if and only if it is open in $\Ind_P^G(\sO_0)$. Also note that the set $\sO\cap (\sO_0+\mathfrak u)$ is nonempty for all such orbits $\sO$ (see ?).}

For a closed subgroup $H\subseteq G$ and an element $x\in \g_\R$, write 
\[
\mathrm Z_H(x):=\{g\in H:\Ad(g)x=x\} \quad(\textrm{$\Ad$ stands for the adjoint representation}). 
\]
According to \cite[103]{B.Orbit} (see also \cite[p443]{MW}), for every $x\in \sO \cap (\sO_0 +\fuu_\R)$, $\mathrm Z_G(x)/ \mathrm Z_P(x)$ is a finite group which is independent of $x$ up to conjugation by $P$.  %and $\sO$ is induced from $\sO_0$. 
\begin{defn}\label{def:mult}
The \emph{multiplicity} of $\sO$ is defined to be 
\[
m(\sO) := \#\left(\mathrm Z_G(x)/ \mathrm Z_P(x)\right) \qquad (\# \textrm{indicates the cardinality of a finite set}),
\]
where $x\in {\sO}\cap(\sO_0+\mathfrak u_\R)$. 
\end{defn}

%The above quantity $m(\sO)$ is independent of the choice of $x\in {\sO}\cap(\sO_0+\mathfrak u)$ . 

Define the induced set of $\sO_0$ with multiplicities to be the formal sum 
\[\Ind_P^G(\sO_0) := \sum_{\sO} m(\sO) \sO,\] 
where $\sO$ runs over all nilpotent $G$-orbits induced  from $\sO_0$. 

Fix a $G$-invariant non-degenerate symmetric bilinear form $\la\,,\,\ra_{\g_\R}$ on $\g_\R$. We identify $\g_\R$ with $\mathrm i\g_\R^*$ ($\mathrm i:=\sqrt{-1}\in \C$ and $\,^*$ indicates the dual space) via the linear isomorphism
\be\label{idg}
  \g_\R\rightarrow \mathrm i\g_\R^*, \quad x\mapsto (y\mapsto \mathrm i\la x,y\ra_{\g_\R}). 
\ee
Recall that the wavefront cycle of a Casselman-Wallach  representation  of $G$ is a formal sum of nilpotent $G$-orbits in $\mathrm i\g_\R^*$. Via the above identification, it is also viewed as a formal sum of  nilpotent $G$-orbits in $\mathrm \g_\R$. By using the restriction of the form $\la\,,\,\ra_{\g_\R}$ to $\frak m_\R\times \frak m_\R$, similarly we identify $\fmm_\R$ with $\mathrm i\fmm_\R^*$. 

We recall the following result of Barbasch. 
\begin{thm}[{\cite[Corollary 5.0.10]{B.Orbit}}]\label{thm:A3}
 Suppose a Casselman-Wallach  representation $\pi$ of $M$ has wavefront cycle $\sum_{\sO_i} c_{\sO_i} \sO_i$. 
 Then $\Ind_P^G (\pi)$ has wavefront cycle 
 $\sum_{\sO_i} c_{\sO_i}\Ind_P^G (\sO_i)$. 
\end{thm}


\subsection{Computation of the induced orbits and multiplicities}
In the rest of this appendix we spell out the computation of nilpotent orbit induction used in  the proof of  \Cref{lem:ind00}. Let $\epsilon,\dot \epsilon\in\{1,-1\}$ and let $(V, \la\,,\,\ra, J)$ be a triple  satisfying the following conditions:
  \begin{itemize}
...
\item   $\inn{J u}{Jv}=
  \overline{\inn{u}{v}},\quad$  for all $u,v\in V$.
  \end{itemize}
Note that the triple $(V, \la\,,\,\ra, J)$ is part of the data specifying a classical space (see Section \ref{classical}). 


Denote by $G_{\C}$ the isometry group of $ (V, \la\,,\,\ra)$, which is a complex orthogonal group if $\epsilon=1$ and a complex symplectic group if $\epsilon=-1$. Denote by  $G:=\mathrm{G}(V):=G_{\C}^{J}$ the centralizer of $J$ in  $G_{\C}$, which is a classical group as in Section \ref{classical}.  

Suppose that we are given a $J$-stable decomposition
\begin{equation}\label{eq:WittDecomp}
V=E\oplus V_0\oplus E',
\end{equation}
where $E$ and $E'$ are non-zero, totally isotropic and paired perfectly by $\langle\ ,\ \rangle$, and $V_0$ is the orthogonal complement of $E\oplus E'$.
Let $P=M\ltimes U$ be the parabolic subgroup of $G$ stabilizing $E$, with unipotent radical $U$ and Levi subgroup
\[
M=G_0\times \GL(E)^J,
\]
where $G_0:=\mathrm{G}(V_0)$, and $ \GL(E)^J$ is the centralizer of $J$ in the general linear group $\GL(E)$ on $E$ (similar notation will be used without explanation). Set 
$\mathfrak u_\R:=\Lie(U)$ and $\mathfrak g_{0\R}:=\Lie(G_0)$. 


A $(-\epsilon)$-symmetric bilinear form $T$ on $E'$ is called $J$-compatible if 
\begin{equation} \label{hermep}
T(J u, Jv)=
  \overline{T(u,v)}\quad\textrm{  for all $u,v\in E'$}.
\end{equation}
Let $\mathrm{Herm}_{-\epsilon}(E')$ denote the space of  $J$-compatible  $(-\epsilon)$-symmetric  bilinear forms on $E'$.
%The set of  $\GL(E)^J$-conjugate classes of $J$-compatible $(-\epsilon)$-symmetric bilinear forms only depends on the dimension of $E$, we denote the set to be $\Iso_{-\epsilon,\dim E}$
By abuse of notation, for every $T\in \mathrm{Herm}_{-\epsilon}(E')$, we still use $T: E'\rightarrow E$ to denote the linear map such that
\[
  \langle T(u), v\rangle= T(u, v)\quad\textrm{  for all $u,v\in E'$.}
\]
With respect to the decomposition  \eqref{eq:WittDecomp}, we have an identification %the Lie algebra $\mathfrak u_\R$ is identified with$\Hom_J(E',V_0)\oplus\mathrm{Herm}_{-\epsilon}(E')$ by sending $(S,T)$ to
\begin{equation}\label{eq:YCB}
\Hom_J(E',V_0)\oplus\mathrm{Herm}_{-\epsilon}(E')\xrightarrow{\sim} \frak u_\R, \quad (S,T)\mapsto Y_{S,T}:=
\begin{bmatrix}
0&-S^* &T\\
0&0&S\\
0&0&0
\end{bmatrix}.
\end{equation}
Here $\Hom_J(E',V_0)$  denotes the space of $J$-intertwining linear maps
from $E'$ to $V_0$ (similar notation will be used without explanation), and 
 $S^*\in\Hom_J(V_0,E)$ is specified by
\begin{equation}\label{eq:Cvee}
\langle S^* v,u\rangle=\langle v,S u\rangle \qquad \textrm{for all } v\in V_0,\ u\in E'.
 \end{equation}
%Denote by $M$ the Levi subgroup of $P$ stabilizing the decomposition \eqref{eq:WittDecomp}; put $G_0=\mathrm{G}(V_0)$ and $\mathfrak g_0:=\Lie(G_0)$.  Thus $\mathfrak g_0$ is viewed as a Levi summand of $\mathfrak p=\Lie(P)$.

%Let $\Hom_J(E',V_0)$ denote the space of $J$-intertwining linear maps from $E'$ to $V_0$. 




Fix a nilpotent $G_0$-orbit $\sO_0\subset\mathfrak g_{0\R}$, which is also viewed as a nilpotent $M$-orbit of $\mathfrak m_\R:=\Lie(M)$.  We also fix an element $X_0\in \sO_0$, which is a $J$-intertwining linear map $X_0: V_0\rightarrow V_0$. Denote by $R_0$   the radical of the $\epsilon$-symmetric bilinear form $\la\, ,\,\ra$ on $\Ker X_0$. 
By using the Jacobson–Morozov theorem, it is easy to see that 
\[
  \dim R_0=\dim(\Ker X_0^2/\Ker X_0).
\]
For application to the proof of Proposition~\ref{lem:ind00}, we assume in the rest of this appendix that \begin{equation}\label{eq:r-gd0}
\dim E\geq \dim R_0.
\end{equation}
% and that
% \be\label{even}
%   \textrm{if $\epsilon=\dot \epsilon=-1$ so that $G$ is a real symplectic group, then $\dim (\Ker X_0)-\dim E $ is even.}
% \ee

The conjugate linear automorphism $J:V\rightarrow V$ induces a conjugate linear automorphism $J:(\Ker X_0)/R_0\rightarrow (\Ker X_0)/R_0$, and the $\epsilon$-symmetric bilinear form $\la\,,\,\ra$ induces a non-degenerate  $\epsilon$-symmetric bilinear form on $(\Ker X_0)/R_0$. Similar to the group $G=\mathrm{G}(V)$, we have the classical group $\mathrm{G}((\Ker X_0)/R_0)$.

An  $\epsilon$-symmetric bilinear form $T'$ on a complex vector space $V'$ is said to be regular if one of the following two conditions is satisfied:   
\begin{itemize}
    \item $T'$ is non-degenerate;
    \item $\epsilon =-1$, $V'$ is odd dimensional, and the radical of $T'$ is one-dimensional.  
\end{itemize}
 When $V'$ is equipped with an $\epsilon$-symmetric bilinear form, a subspace $V''$ of $V'$ is said to be regular if the $\epsilon$-symmetric form is regular when restricted to $V''\times V''$.

%We define a subset $\Hom_J^{\max}(E',V_0)$ of $\Hom_J(E',V_0)$ as follows.  

When $\dim E \geq \dim (\Ker X_0)$, we define   
 \[
 \Hom_J^{\max}(E',V_0):=\{S\in \Hom_J(E',V_0)  \,:\,  S^*|_{\Ker X_0} \text{ is injective}\}. 
\]
%Here and henceforth $\Im$ indicates the image of a map. 

When $\dim E < \dim (\Ker X_0)$, we define $\Hom_J^{\max}(E',V_0)$ to consist of those $S\in \Hom_J(E',V_0)$ such that 
\[
\textrm{$S^*|_{R_0}$ is injective, and $S^*|_{\Ker X_0}$ is surjective with regular kernel}.
\]
%\begin{eqnarray*}   &&\Hom_J^{\max}(E',V_0) \\  &:=& \{S\in \Hom_J(E',V_0) \,:\,  S^*|_{\Ker X_0}  \text{ is surjective and its kernel is regular and has zero intersection with $R$}\}.\end{eqnarray*} 
%{\color{red} May delet this now: Here a subspace $V'$ of an $\epsilon$-symmetric space is called regular if the $\epsilon$-symmetric form is regular when restricted to $V'\times V'$. }
For each $S\in \Hom_J^{\max}(E',V_0)$, let  
\[
V_S := \textrm{the image of $\Ker (S^*|_{\Ker X_0})$ under the quotient map $\Ker X_0\rightarrow  (\Ker X_0)/R_0$}.
\]
 This is a $J$-stable subspace of $(\Ker X_0)/R_0$ of dimension $d$, where 
\[
d:=\dim (\Ker X_0) -\dim E, 
\]
and it is regular.
...
Denote by 
 \[
\mathrm{Gr}_{d}^{\mathrm{reg}} ((\Ker X_0)/R_0):=\{\textrm{$J$-stable regular subspaces of $(\Ker X_0)/R_0$ of dimension $d$}\}.
\]
This set carries an obvious action of $\mathrm{G}((\Ker X_0)/R_0)$.

%S^*|_{R} \text{ is injective, and } \Ker S^*\text{ is a
%When $\dim \ker X_0 > \dim E \geq \dim (\ker X_0^2/\ker X_0)$, let  
%\[\Hom_J^{\max}(E',V_0) := \{S\in \Hom_J(E',V_0) \,:\,  S^*|_{\Ker X_0}  \text{ is surjective } S^*|_{R} \text{ is injective, and } \Ker S^*\text{ is as non-degenerate as possible}\}.\]Here $\bar S : E' \to V_0 \to V_0 /\Im X_0$. 
%Here $R$ denotes the radical of the $\epsilon$-symmetric bilinear form $\la\, ,\,\ra$ on $\Ker X_0$.
%Fix a  $J$-intertwining linear map $S: E'\rightarrow V_0$ such that Let $\Hom_J^{\max}(E',V_0)$ denotes the set of all $S\in \Hom_J(E',V_0)$ with maximal possible rank of the composition of
%\be\label{defhommaz}E'\xrightarrow{\ \ S \ \ } V_0\xrightarrow{\textrm{quotient map}} V_0/ \Im X_0. \ee

The set $X_0+ \Hom_J^{\max}(E',V_0)$,  viewed as a subset of the  quotient set  $(X_0+\frak u_\R)/\mathrm{Herm}_{-\epsilon}(E')$ via \eqref{eq:YCB}, carries an obvious action of 
\[
H:= (\GL(E')^J\times \mathrm Z_{G_0}(X_0))\ltimes U\subset P
\]
...
\begin{prop}\label{prop:A4}
 \noindent (a) Under the Euclidean topology, the set $\Hom_J^{\max}(E',V_0)$ is open dense in $\Hom_J(E',V_0)$  and  every $H$-orbit  in $X_0+\Hom_J^{\max}(E',V_0)$  is open in $X_0+\Hom_J(E',V_0)$. 
 
 \noindent (b) Suppose that $\dim E \geq \dim (\Ker X_0)$. Then $H$ acts on $X_0+\Hom_J^{\max}(E',V_0)$ transitively. 
 
 \noindent (c) Suppose that $\dim E < \dim (\Ker X_0)$. 
 Then the map $S\mapsto V_S$ yields a bijection between the set of $H$-orbits
 in $X_0+\Hom_J^{\max}(E',V_0)$ to the set of  $\mathrm{G}((\Ker X_0)/R_0)$-orbits in $\mathrm{Gr}_{d}^{\mathrm{reg}} ((\Ker X_0)/R_0)$. 
 %$J$-stable regular subspaces of $(\Ker X_0)/R$ of dimension $\dim( \Ker X_0)-\dim E$. %isometry class of $V_S$.  


\end{prop}

%  An element $S \in \Hom_J^{\max}$ is called in a normal form if 
%  \[
%  S^{-1}(\Im X_0)  = \Ker S. 
%  \]
%  By the definition of $\Hom_J^{\max}(E',V_0)$ there is a normal form for each $H$-orbit in $X_0 + \Hom_J^{\max}(E',V_0)$.  


Fix  a $J$-stable complement $L_0$ of the image $\Im X_0$ of $X_0$ so that
\begin{equation}\label{eq:L0}
V_0=L_0 \oplus \Im X_0\qquad (\textrm{$\Im$ indicates the image of a map}).
\end{equation}
View $\Hom_J(E',L_0)$ as a subspace of $\Hom_J(E',V_0)$ and 
let 
\[
\Hom_J^{\max}(E',L_0)  := \Hom_J(E',L_0) \cap  \Hom_J^{\max}(E',V_0) . 
\]
Then each $H$-orbit in  $X_0 + \Hom_J^{\max}(E',V_0)$ intersects 
$X_0 +\Hom_J^{\max}(E',L_0)$. 


For every  $S\in \Hom_J^{\max}(E',L_0)$, define 
%\[
%\Herm_{-\epsilon}^{\mathrm{reg}}(E',S) := \{T \in \Herm_{-\epsilon}(E') \,:\, 
%T|_{\Ker S\times \Ker S} \text{ is regular}\}. 
%\]
\[
\Herm_{-\epsilon}^{\mathrm{reg}}(E')_S := \{T \in \Herm_{-\epsilon}(E') \,:\, 
T|_{\Ker S\times \Ker S} \text{ is regular}\}.
\]
%Two forms $T'_1$ and $T'_2$ are said to be congruent if there is $g \in \GL(V')^J$ such that $T'_1(g v_1,g v_2) = T'_2(v_1,v_2)$ for $v_1,v_2\in V'$. 
%if either  it is  non-degenerate or  its radical is one-dimensional if $-\epsilon = -1$ and $\dim V'$ is odd, and non-degenerate otherwise.  
%The congruent class of $J$-compatible maximally non-degenerate $\epsilon$-symmetric bilinear forms on $\Ker S$. 
%only depends on 
%$d = \Ker S = \max(0,\dim E - \dim \Ker X_0)$, let 
%\[
%\Iso_{-\epsilon,d}
%\]
%denote the set.
%Let 
%\[
%\cU := \{ X_0 + Y_{S,T} : S\in \Hom_J^{\max}(E', L_0),\, T \in \Herm^{\mathrm{reg}}_{-\epsilon}(E')_S
%\}.
%\]
Write 
\[
\cU := \{Y_{S,T} : S\in \Hom_J^{\max}(E', L_0),\, T \in \Herm^{\mathrm{reg}}_{-\epsilon}(E')_S
\}.
\]


The following proposition also follows from elementary linear algebra. 

\begin{prop}\label{prop:A5}
\noindent (a) Under the Euclidean topology, the set $H\cdot  (X_0 + \cU)$ is open dense in $X_0 + \fuu_\bR$ and  every  $H$-orbit in $H\cdot (X_0+ \cU)$ is open in $X_0 + \fuu_\bR$.  

\noindent (b) 
Fix $S\in \Hom_J^{\max}(E', L_0)$ and let $T_1, T_2 \in \Herm_{-\epsilon}^{\mathrm{reg}}(E')_S$. Then 
$X_0 + Y_{S,T_1}$ and $X_0 + Y_{S,T_2}$ are in the same $H$-orbit 
%Then 
%\[
%H \cdot (X_0 + Y_{S,T_1}) = H \cdot (X_0 + Y_{S,T_2}) 
%\]
if and only if $T_1|_{\Ker S\times \Ker S}$ and $T_2 |_{\Ker S\times \Ker S}$ are conjugate under $\GL(\Ker S)^J$. 
\end{prop}

We are now ready to enumerate the representatives of the induced orbits. 


Suppose that $\dim E \geq \dim (\Ker X_0)$. Then there exists a surjective linear map $S\in \Hom_J(E', L_0)$. Note that the surjectiveness is equivalent to the condition that  $S\in \Hom_J^{\max}(E',L_0)$. 
%Pick {\color{red} Pick any $S \in\Hom_J^{\max}(E',V_0)$. Namely we pick} any surjective map $S \in \Hom_J(E',L_0)$, {\color{red} to be} viewed as an element of $\Hom_J(E',V_0)$, {\color{red} as before}.  
Fix a $J$-stable decomposition 
\be\label{decep}
E' = \Ker S \oplus E''.
\ee
 Each  bilinear form $T \in \mathrm{Herm}_{-\epsilon}(\Ker S)$ uniquely extends to a bilinear form in $\mathrm{Herm}_{-\epsilon}(E')$ whose radical contains $E''$. By abuse of notation, still denote by $T$ this extended bilinear form. In this way we view $\mathrm{Herm}_{-\epsilon}(\Ker S)$ as a subspace of $\mathrm{Herm}_{-\epsilon}(E')$. 
 
 Let $\mathrm{Herm}_{-\epsilon}^{\mathrm{reg}}(\Ker S)$ denote the set of all regular forms in $\mathrm{Herm}_{-\epsilon}(\Ker S)$, which carries an obvious action of $\GL(\Ker S)^J$. 
 %We may view $\mathrm{Herm}_{-\epsilon}^{\mathrm{reg}}(\Ker S)$ as a subspace of $\mathrm{Herm}_{-\epsilon}(E')$ by requiring $E''$ as in \eqref{decep} to be in its radical. 
%In this way, each  $T \in \mathrm{Herm}_{-\epsilon}^{\mathrm{reg}}(\Ker S)$ yields an element in $\mathrm{Herm}_{-\epsilon}(E')$, still denoted by $T$.}
%let $T_{T'}$ denote the extension of $T'$ to $E'\times E'$ such that $E''$ is contained in its radical, and set  % by $T_{T'}(v_1 + v_2, v_3 + v_4) = T'(v_1,v_3)$ for $v_1,v_3\in \Ker S$ and $v_2, v_4\in E''$.
Set \[
X_{S,T}:=X_0+Y_{S,T}\in \g_\R,\qquad T\in \mathrm{Herm}_{-\epsilon}^{\mathrm{reg}}(\Ker S)\subset \mathrm{Herm}_{-\epsilon}(E').
\]


\begin{prop}\label{prop:A6}
Suppose that $\dim E \geq \dim (\Ker X_0)$. Let $S\in \Hom_J^{\max}(E',L_0)$. 

\noindent
(a) For each $T\in \mathrm{Herm}_{-\epsilon}^{\mathrm{reg}}(\Ker S)$, the orbit $G\cdot X_{S,T}$ is independent of the choice of the decomposition \eqref{decep}. Moreover, the  map $T\mapsto
 X_{S,T}$ induces a bijection between the set of  $\GL(\Ker S)^J$-orbits in $\mathrm{Herm}_{-\epsilon}^{\mathrm{reg}}(\Ker S)$ with the set of nilpotent $G$-orbits induced from $\sO_0$. 

% \noindent
% (b)
% The singed Young diagram of 
% $G \cdot X_{S,T'}$ is given by 
% \eqref{eq:IndSYD1} and \eqref{eq:IndSYD2}
% (with $d = t_0>0$). 
% More presciely,  if the form $T'$ has signature $(a,b)$, the it corresponds to the orbit 
% \[
% \sO_0\circledast(a,b)\circledast(d-(a+b),d-(a+b)).
% \]

\noindent
(b) For each $T\in \mathrm{Herm}_{-\epsilon}^{\mathrm{reg}}(\Ker S)$, the multiplicity  
\begin{equation}\label{countmul}
m(G\cdot X_{S,T})=
\begin{cases}
2, & \text{if $\epsilon = 1$ and $\Ker S$ is odd dimensional;}\\
1, & \text{otherwise}.
\end{cases}
\end{equation}
\end{prop}
\begin{proof}
The first assertion of part (a) is routine to check, while the second assertion  follows from Propositions \ref{prop:A4} and \ref{prop:A5}.

Now we consider part (b). 
In the second case of \eqref{countmul}, $\Ker (X_{S,T})=E$.  Every element of $\mathrm Z_G(X_{S,T})$ therefore preserves $E$, hence lies in $P$.  Thus $\mathrm Z_G(X_{S,T})=\mathrm Z_P(X_{S,T})$, and
\[
m(G\cdot X_{S,T})=1.
\]

In the first case of \eqref{countmul}, write $R$ for the radical of $T$, which is a  one-dimensional subspace of $\Ker S$. 
%Let $R' := \Ker X_{S,T'}\cap \Im X_{S,T'}$.
The centralizer $\mathrm Z_G(X_{S,T})$ acts transitively on the set $\{E,R + (\Ker X_{S,T}\cap \Im X_{S,T})\}$, while the subgroup preserving $E$ is precisely $\mathrm Z_P(X_{S,T})$. Consequently
\[
m(G\cdot X_{S,T})=[\mathrm Z_G(X_{S,T}):\mathrm Z_P(X_{S,T})]=2.
\]
\end{proof}

Now we suppose that $\dim E < \dim (\Ker X_0)$. 
The decomposition \eqref{eq:L0} induces a $J$-stable decomposition
\[
V_0 = L_0^\perp \oplus \Ker X_0,
\]
where 
\[
L_0^\perp:=\{u\in V_0\,:\, \la u, v\rangle=0 \textrm{ for all }v\in L_0\}.
\]
For a given $A\in \mathrm{Gr}^{\mathrm{reg}}_d((\Ker X_0)/R_0)$, fix a $J$-stable decomposition 
\be\label{decomaa}
\Ker X_0 = R_0 \oplus A' \oplus A''
\ee
such that the quotient map $\Ker X_0 \to \Ker X_0/R_0$ sends $A'$ onto $A$. 
%Fix an isomorphism $R\oplus A'' \to E$ and extends 
Choose any $S_A\in \Hom_{J}(E',V_0)$ such that 
\be \label{chSA}
\textrm{$S_A^*|_{ L_0^\perp \oplus A' } = 0\quad$ and $\quad S_A^*|_{R_0\oplus A'' } $ is bijective.}
\ee
Then $S_A\in \Hom_J^{\max}(E',V_0)$.

As before, set
\[X_{S_A,0}:=X_0+Y_{S_A,0}\in \g_\R, \qquad A \in \mathrm{Gr}^{\mathrm{reg}}_d((\Ker X_0)/R_0).\]


\begin{prop}\label{prop:A7}
Suppose that $\dim E < \dim (\Ker X_0)$. 

\noindent (a) For each $A\in\mathrm{Gr}^{\mathrm{reg}}_d((\Ker X_0)/R_0) $, the orbit $G\cdot X_{S_A,0}$ is independent of the  choice of the decomposition \eqref{decomaa} and 
the choice of $S_A$ in \eqref{chSA}. Moreover, the map $A\mapsto
X_{S_A,0}$ induces a bijection between the set of $\mathrm{G}((\Ker X_0)/R_0)$-orbits in $\mathrm{Gr}^{\mathrm{reg}}_d((\Ker X_0)/R_0)$ with the set of nilpotent $G$-orbits induced  from $\sO_0$. 

\noindent (b) For each $A\in\mathrm{Gr}^{\mathrm{reg}}_d((\Ker X_0)/R_0) $, 
the multiplicity
\[
m(G\cdot X_{S_A,0}) = \begin{cases}
2 , & \text{if $\epsilon=-1$ and $d$ is odd};\\
1, & \text{otherwise}.
\end{cases}
\]
\end{prop}
\begin{proof}
The proof is similar to that of \Cref{prop:A6} and is omitted. 
\end{proof}


%To prove \Cref{lem:ind00}, we identify the Lie algebra $\fgg$ of $G_\bC$ with the dual space $\fgg^*$ via the trace form. 

Note that the trace form on the Lie algebra $\g$ of $G_\C$ is real-valued when restricted to $\g_\R\times \g_\R$, where $\g_\R$ denotes the Lie algebra of $G$. Thus, by \eqref{idg}, the trace form yields an identification of $\fgg_\R$ with  $\mathrm i \g_\R^*$, and we may view the wavefront cycle of a Casselman-Wallach representation  of $G$ as a formal sum of nilpotent $G$-orbits in $\mathrm \g_\R$. Thus, in view of \Cref{thm:A3}, Propositions \ref{prop:A6} and \ref{prop:A7} provide an explicit description of the wavefront cycle of the relevant induced representations. 
...
\bib{AMVV}{article}{
       ...
       author = {Vogan, D. A.}
        tsitle = {Unipotent representations of real exceptional groups},
...
\bib{LMBM}{book}{
  ...
  % volume={107},
   ...
   date={2027},
   pages={xxi+280},    %journal = {arXiv:2108.03453},
}
...
\bib{MW}{article}{
  ...
  title={Mod{\`e}les de Whittaker d{\'e}g{\'e}n{\'e}r{\'e}s pour des groupes $p$-adiques},
  journal={Math. Z.},
  volume={196},
  date={1987},
  pages={427--452},
