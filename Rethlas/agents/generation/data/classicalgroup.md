...
Optionally, also prove uniqueness: if another quadruple satisfies the same conditions for the same \(\mathsf s\), then there is a complex-linear isomorphism preserving \(\langle\ ,\ \rangle\), \(J\), and \(L\).
...
5. optional uniqueness proof if feasible.
