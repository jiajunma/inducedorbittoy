...
\]
coming from the two perfect pairings.  The first rank-open condition is
\(\phi_C\) surjective.  Then
\[
L_C:=\ker\phi_C
\]
has dimension \(c_1-\kappa=d\).  The second rank-open condition is
\(\phi_C|_R:R\to E\) injective, equivalently \(L_C\cap R=0\).  Hence the
quotient map \(q:\ker X_0\to\K\) sends \(L_C\) isomorphically onto a
\(d\)-plane
\[
A_C:=q(L_C)\subset\K .
\]
The last rank-open condition is \(A_C\in\Amr\).  Thus the desired open subset
is
\[
\Omega=\{X_{C,B}:\phi_C\text{ is surjective},\
\phi_C|_R\text{ is injective},\ A_C\in\Amr\}.
\tag{2}
\]

To see this is a nonempty polynomial open set, choose \(V_+\subset V_0\) with
\[
V_0=V_+\oplus\im X_0,\qquad V_+\cap\ker X_0=V_1.
\]
For \(C:E'\to V_0\), write \(S_C:E'\to V_+\) for its quotient modulo
\(\im X_0\).  Then \(\phi_C\) is the transpose of \(S_C\), so
\[
...
\]
is surjective.  Thus the first generic locus is represented by injective maps
\(S:E'\to V_+\).  The further condition \(q(\ker\phi_C)\in\Amr\) can be read as
maximal possible rank of the Gram matrix
\[
(u,v)\mapsto \langle S(u),S(v)\rangle_0
\]
among such \(S\) with \(\overline S\) surjective.  These are exactly
nonvanishing-minor conditions, and they are nonempty by choosing \(S\) with
maximal possible Gram rank.

Now define the normal forms only after \(A\) has appeared from the open
conditions.  For \(A\in\operatorname{Gr}_d(\K)\), let
\(W_A\subset V_0/\im X_0\) be the annihilator of \(A\) under
...
Take \(x=X_{C,B}\in\Omega\), and put \(A=A_C\).  A Levi conjugation first makes
\(\phi_C=S_A^\vee|_{\ker X_0}\).  Then \(C-S_A\) has image in \(\im X_0\), so
(1) makes \(C=S_A\).  A second conjugation by some \(u_D\) with
\(D(E')\subset\ker X_0\) kills the remaining \(B\)-block.  Thus every point of
\(\Omega\) is \(P\)-conjugate to \(x_{A_C}\).  Conversely each
\(x_A\) with \(A\in\Amr\) lies in \(\Omega\), so the normal-form conjugations
give
\[
\Omega=P\cdot\{x_A:A\in\Amr\}.
\tag{4}
\]
The same argument shows that \(x_A\) is independent, up to \(P\)-conjugacy, of
the choices of \(V_A,S_A\).  If two subspaces \(A,A'\subset\K\) are related by
an ambient isometry of \(\K\), that isometry lifts to an element of
\(Z_{G_0}(X_0)\), hence \(x_A\) and \(x_{A'}\) are \(P\)-conjugate.

Moreover \(\Omega\) is a finite disjoint union
\[
...
\(Z_{G(W)}(x_A|_W)\subset P(W)\).  By (10),
\[
